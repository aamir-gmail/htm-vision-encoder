...
def predict_and_reduce(new_model=None):
    ...
    X_hat = predections
    pickle.dump(X_hat, open('data/x_hat_128_v5.pkl', mode='wb'))

def sacler_data_randonscaler():
    pooler_data = []
    data = pickle.load(open('data/x_hat_128_v5.pkl', mode='rb'))
    data1 = data[:,0:64]
    data2 = data[:,64:128]
    data1 = TSNE(n_components=2).fit_transform(data1)
    data2 = TSNE(n_components=2).fit_transform(data2)
    col1 = data1[:, 0:1].flatten()
    col2 = data1[:, 1:2].flatten()
    col3 = data2[:, 0:1].flatten()
    col4 = data2[:, 1:2].flatten()
    ...
    parameter4.size = 1000
    parameter4.sparsity = 0.02
    parameter4.resolution = 0.88
    ...
    for _x1, _x2 , _x3, _x4 in zip(col1, col2, col3, col4):
        ...
        x_x4 = rsc4.encode(_x3)
        pooler_data.append(SDR(4000).concatenate([x_x1, x_x2, x_x3,x_x4]))
    return pooler_data
...
def spatial_pooler_encoer(pooler_data):
    sp1 = SpatialPooler(
        inputDimensions=(4000,),
        columnDimensions=(4000,),
        potentialPct=0.85,
