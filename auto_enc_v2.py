...
def save_model(model):
    ...
    with open('model/embedding_v2.json', 'w') as json_file:
        json_file.write(model_json)
    # serialize weights to HDF5
    model.save_weights("model/embedding_v2.h5")
    print("Saved model to disk")


def load_model():
    json_file = open('model/embedding_v2.json', 'r')
    loaded_model_json = json_file.read()
    json_file.close()
    loaded_model = model_from_json(loaded_model_json)
    # load weights into new model
    loaded_model.load_weights("model/embedding_v2.h5")
...
def predict_and_reduce(new_model=None):
    ...
    pickle.dump(X_hat, open('data/x_hat_v2.pkl', mode='wb'))

def sacler_data_randonscaler():
    pooler_data = []
    data = pickle.load(open('data/x_hat_v2.pkl', mode='rb'))
